...
# converting the timestamp string to datetime object
    ...
    if not after_opening_range_breakdown.empty:
        if symbol not in existing_orders_symbols:
            ...
            limit_price_stuff = after_opening_range_breakdown.iloc[0]['close']
            print(limit_price_stuff)
            message = f"placing short order for {symbol} at {limit_price_stuff}, closed_below {opening_range_low} at {after_opening_range_breakdown.index.time[0]}"
            print(message)
            ...
            try:
                order = LimitOrderRequest(symbol = symbol, side=OrderSide.SHORT, type=OrderType.LIMIT, limit_price = limit_price_stuff, qty = calculate_quantity(10000, limit_price_stuff), time_in_force= TimeInForce.DAY,  stop_loss=stop_loss_req, take_profit=take_profit_req)
